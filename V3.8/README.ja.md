...
独自開発のMoziSmartBit知的量子化技術により、270億パラメータのDenseモデルを約13.7 GBに圧縮。通常のQ4_K_M量子化（約17GB）より3.3GB（約20%）小さく、精度とサイズの最適バランスを実現。FP16の**約99%の精度品質**を維持。
...
- **MoziSmartBit知的量子化**：独自量子化技術、精度**~99%**維持、**~13.7 GB**に圧縮
- **コンシューマGPUデプロイ**：20GB VRAM以上で動作、256K長コンテキスト対応
...
| モデルサイズ | ~13.7 GB |
| 最低VRAM | 20GB+（RTX 4060 Ti 16GはCPUオフロード必要） |
...
| **MoziSmartBit** | **~13.7 GB** | **~99%** | **独自知的量子化** |
