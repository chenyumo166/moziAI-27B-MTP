...
MoziAI-27B-3.8 是由中国财经大V陈雨墨团队开发的本地开源多模态AI大模型（增强金融领域、支持视觉、工具调用、消费级显卡本地部署）。moziAI-27B-3.8 基于开源底座 Qwen3.8-27B（Dense 27B 架构，MIT 许可），结合陈雨墨团队自主研发的（金融数据 + 金融领域能力 + 训练方法 + 动态七维思考体系 + 智能体LOOP反思迭代机制 + 混合量化算法 MoziSmartBit）开发而成。降低个人与企业的本地部署门槛，授权免费商用。因本地消费级显卡可部署使用，节约大量云端token成本，实现7×24小时token自由并且确保本地数据隐私与安全。

**动态七维思考体系 + 智能体LOOP迭代机制**：MoziAI 自研的核心推理模式。智能判断任务复杂度，简单任务启动二维思考快速回答，中度复杂任务启用五维思考，高度复杂任务启用完整七维思考+LOOP反思迭代机制。试图挑战比自己体积大几十倍的万亿级参数模型的复杂任务的有效解决能力，又不失简单任务的轻快响应。让本地模型也能养成人类资深专家"先想后做"的能力，这套自研的核心推理模式对比同等体积模型极具特色。

**通过自研的 MoziSmartBit 智能量化技术**，270亿参数的密集模型被压缩至约12.79 GB，比常规的Q4_K_M量化模型（约17 GB）小约4.2 GB（约25%）；在精度与大小之间实现了最佳平衡，提供~99%的FP16精度质量。

本模型除了保留AI大模型的通用能力外，还增强了：金融垂直领域应用，金融问答、量化编程、工具调用和通用编程，兼容各种agent平台调用。

模型研发者陈雨墨常把本模型用于本地金融数据分析、量化策略研发、市场调研、任何的文章编写、整体项目开发推进、通用程序编写，OpenClaw/Hermes执行256K长上下文的复杂任务。

支持 llama.cpp、Ollama、LM Studio 等主流推理框架免费本地部署

**发布日期：2026-08-30** | **版本：V3.8**

## 模型特色

- **🧠 动态七维思考体系**：MoziAI 自研的核心推理框架。面对任何任务，模型先输出 **moziAI-Think** 标记，按任务复杂度动态展开结构化思考——从简单问答的"理解任务 + 资源需求"两维速答（Level 0），到分析诊断的五维评估（Level 1），再到复杂开发/策略设计的全七维深度推演（Level 2）：①理解任务 ②复杂度评估 ③依赖关系 ④风险评估 ⑤资源需求 ⑥验收标准 ⑦执行策略。
- **🧠 智能体LOOP迭代机制**：MoziAI 对于复杂任务，模型自动进入 **moziAI-Loop** 迭代模式：第1轮执行+评估 → 第2轮调整+验证，确保输出经过自我校验后才给出最终答案，而非一次性生成。这意味着模型不只是"回答问题"，而是像资深工程师一样拆解问题→评估方案→执行→反思→优化，显著提升复杂任务的准确性和可执行性。简单问答和任务则自动关闭Loop迭代机制。
- **🧠 MoziSmartBit 智能量化**：MoziAI 自主研发的分层智能量化，在精度与大小之间达到最佳平衡，压缩至约 12.79 GB，并保持FP16~99% 的精度
- **🧠 金融垂直领域聚焦**：针对金融问答、量化编程和工具调用的深度优化。金融领域对于模型幻觉的容忍度极低，也证明了moziAI对比通用模型在垂直领域能力进行深度加强。
...
- **多Agent平台支持**：深度适合OpenClaw、Hermes、OpenCode、Cursor、Windsurf、Claude Code、Codex 等国内外主流 AI IDE 与 Agent 框架，原生支持工具调用与多轮任务编排，开箱即可
...
| **MoziSmartBit** | **~12.79 GB** | **~99%**  | **本模型采用自研智能量化方式** |
| Q4_K_M         | ~17 GB      | ~98%     | GGUF 标准 4bit      |
| Q5_K_M         | ~20 GB     | ~99%     | 更高精度              |
| Q6_K            | ~23 GB     | ~99.5%   | 近无损              |
| Q8_0            | ~31 GB     | ~100%    | 无损失              |

> MoziAI V3.8 采用 MoziSmartBit 智能量化方案，在保持约99%精度的同时，将270亿参数Dense模型压缩至约12.79 GB，压缩比达4.0x，兼顾推理质量与部署门槛，更适合消费级显卡本地部署

## MoziSmartBit 智能量化技术

传统量化方案对所有层使用统一精度，而陈雨墨团队自研的**MoziSmartBit 智能量化**针对 Dense 模型的结构特点，采用智能差异化量化策略，在体积与精度间取得最优平衡，模型质量高于 Q4_K_M 格式，同时体积仅占12.79 GB，压缩比达4.0x，精度保持约99%。
...
由于模型文件较大（~12.79 GB），模型权重托管于多个社群平台：
